...
export function GrievanceList({
  ...
  const colors = ["text-red-600 font-bold", "text-black font-bold", "text-gray-800 font-normal"];
  const columns = [
    ...(isRepeat ? ['S No.'] : []),
    "Registration No",
    ...(isRepeat ? ['Count'] : []),
  ...
  const capitalize = sentance => sentance.trim().split(' ').map(word => word.substring(0, 1).toUpperCase() + word.substring(1).toLowerCase()).join(' ')
  ...
  return (
    <div className={`mb-0 flex flex-col gap-12 ${((titleBarHidden || compactTitle) ? "mt-10" : "mt-12")} ${compactTitle && "mt-3"} ${(scroll ? " h-[100%]" : "")}`}>
      <Card className={scroll ? "h-[100%]" : ""}>
        ...
        <CardBody className={`overflow-x-scroll px-0 pt-0 pb-2 ${(scroll ? "overflow-scroll h-[100%]" : '')} ${(scrollH ? `overflow-scroll h-[${scrollH}]` : '')} shadow-inner`}>
              ...
              <tr>
                {columns.map((el, index) => (
                  ...
                  >
                    <Typography
                      variant="small"
                      className={"text-[14px] font-bold uppercase text-blue-gray-400 flex gap-1 cursor-pointer hover:text-gray-800 " + (sortOrder?.index == index ? 'text-gray-800' : '')}
                  ...
                  <>
                    {
                      ...
                      filteredGrievances?.map((item, key) => {
                        const className = `py-3 px-3 ${key === authorsTableData.length - 1 ? "" : "border-b border-blue-gray-50"}`;

                        // Determine the text style based on the key
                        const textStyle = rowColor + rowRead + ((item?.spam == 1 || title.startsWith('spam')) ? rowSpam : '')
                        return (
                          <tr className="text-md hover:cursor-pointer" onClick={() => handleOpenModal(getRno(item))} key={key}>
                            {
                              isRepeat &&
...
export const Pagination = ({
  ...
  return <div className="flex gap-1 group">
    <Button
      className="bg-white text-blue-800 focus:opacity-100 h-10 px-1 md:h-[50px] rounded-r-sm invisible group-hover:visible"
    ...
    <Button
      className="bg-white text-blue-800 focus:opacity-100 h-10 px-1 md:h-[50px] rounded-r-sm group-hover:rounded-sm"
      disabled={pageno == 1 || disabled}
      ...
      (new Array(VISIBLE_PAGES)).fill('')
        .map((value, index) => {
          ...
          if (btn_page_no >= startPage && btn_page_no <= endPage) {
            return <Button
              className={`bg-white text-blue-800 focus:opacity-100 h-10 px-2 text-lg md:h-[50px] rounded-sm ${pageno == btn_page_no ? 'bg-blue-800 text-white' : ''}`}
    ...
    <Button
      className={`bg-white text-blue-800 focus:opacity-100 h-10 px-1 md:h-[50px] rounded-l-sm ${totalPages != 0 ? 'group-hover:rounded-sm' : ''}`}
...

    {
      totalPages != 0 &&
      <Button
        className="bg-white text-blue-800 focus:opacity-100 h-10 px-1 md:h-[50px] rounded-l-sm invisible group-hover:visible"
        disabled={pageno == totalPages || disabled}
