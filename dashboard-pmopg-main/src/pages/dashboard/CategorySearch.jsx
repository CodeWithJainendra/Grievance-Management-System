...
import { BreadCrumbs, Chart, DateRangePicker, GrievanceListBox, MinistryAutocomplete, StateDistrictAutocomplete, generateTreeFromRca, getBreadCrumbs, buildTreeFromGrievances } from "./CategoricalTree"
...
const FilterBox = ({
    ...
    return <FilterLayout className={`mt-2 p-2 border border-blue-700 rounded transition-all ${!searchType && 'h-0 overflow-hidden p-0'}`}>
        <div className={`${![SearchType.semantic, SearchType.keyword].includes(searchType) && 'hidden'}`}>
            <Input
                type="text"
                label={<div className="input-placeholder bordered">Search Query</div>}
                value={query}
