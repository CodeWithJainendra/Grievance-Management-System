...
import { getGrievancesUsingRegNos, getCategoryTree, getRealTimeRCA, fetchAICategories, fetchAICategoriesHistory, getCriticalCategories } from "@/services/rca";
...
export const RealTimeRCA = () => {
    ...
    const generateAICategories = async () => {
        ...
        try {
            const response = await fetchAICategories({
            ...
            const ai_rca_data = response.data.result.rcadata
    ...
    useEffect(() => {
        async function load() {
            let response = await getRealTimeRCA(filters).catch(console.log)

            try {
                if (response?.data && Object.keys(response?.data).length > 0) {
                    setRcaPath([])
                    setRca(buildTree(response.data))
...
export const Filters = ({
    ...
    const [ministry, setMinistry] = useState(filters.ministry ?
        {
    ...
    return (
        <div className="grid md:grid-cols-4 xl:grid-cols-7 gap-3">
            ...
            <div className="col-span-2 xl:col-span-2 flex flex-col md:flex-row md:justify-end gap-2">
                {
                    CustomActionButton
                    ?? <Button
...

                {
                    ...
                    <Button
                        ...
                        disabled={generatingAICategories}
                    >
                        {
                            generatingAICategories &&
                            <Loader className="mr-2 animate-spin" color="#fff" />
...
const Chart2 = ({
    ...
    return (
        delayedData && delayedData.length > 0 && delayedData[0].title && delayedData[0].value &&
