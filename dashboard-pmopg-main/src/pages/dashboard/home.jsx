...
import { setLoading, useMaterialTailwindController, setSidenavType } from "@/context";
...
export function Home() {
  ...
  // Using the existing controller and dispatch variables
  ...
  const handleThemeChange = () => {
    setSidenavType(dispatch, sidenavType === "dark" ? "white" : "dark");
  };
  ...
  return (
    <div className="mt-12">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Typography variant="small" color="blue-gray">
              {sidenavType === "dark" ? "Dark" : "Light"} Mode
            </Typography>
            <Switch 
              checked={sidenavType === "dark"} 
              onChange={handleThemeChange} 
              color="blue"
            />
          </div>
          <IconButton variant="text" color="blue-gray" className="relative">
            <div className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-red-500 flex items-center justify-center text-[10px] text-white">2</div>
      ...
      {showNotifications.blue && (
        <Alert
          color="blue"
          className="mb-4"
          dismissible={{
            onClose: () => setShowNotifications(prev => ({ ...prev, blue: false })),
          }}
        >
          Welcome to your dashboard! You have 5 new grievances to review.
        </Alert>
      )}

      {showNotifications.green && (
        <Alert
          color="green"
          className="mb-4"
          dismissible={{
            onClose: () => setShowNotifications(prev => ({ ...prev, green: false })),
          }}
        >
          System update completed successfully. All features are now available.
        </Alert>
      )}
