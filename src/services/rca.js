...
export const getGrievancesByRegNosUsingCDIS = async (grievanceRegNos = []) => {
    try {
        // Use consumer_api/get_userdata POST endpoint with IDs
        const url = `https://cdis.iitk.ac.in/consumer_api/get_userdata`;
        
        const requestBody = {
            ids: grievanceRegNos
        };
        
        console.log('🔍 Consumer API Get User Data URL:', url);
        ...
        const data = await response.json();
        console.log('📊 Consumer API Get User Data response:', {
            count: data.count,
            dataLength: data.data?.length,
            requestedRegNos: grievanceRegNos.length
        });
        
        // Transform Consumer API get_userdata response to match expected grievance list format
        const transformedData = (data.data || []).map((item, index) => ({
            // Map Consumer API fields to expected fields - use actual data from API response
            registration_no: item.userId ? `DOCAF/E/2024/${String(item.userId).padStart(7, '0')}` : grievanceRegNos[index] || `CDIS-${Math.random().toString(36).substr(2, 9)}`,
            state: item.stateName || 'Unknown',
            district: item.CityName || 'Unknown', 
            recvd_date: safeDateFormat(item.complaintRegDate),
            closing_date: safeDateFormat(item.updationDate),
            name: item.fullName || 'Unknown',
            ministry: 'DOCAF',
            
            // Additional fields with safe handling
            status: item.status === 1 ? 'Active' : 'Closed',
            userType: item.userType === 0 ? 'Citizen' : 'Other',
            country: item.country || 'India',
            complaintDetails: item.complaintDetails || '',
            complaintType: item.complaintType || 'General',
            complaintMode: item.complaintMode || 'Unknown',
            complaintStatus: item.complaintStatus || 'Unknown',
            categoryCode: item.categoryCode || 'Unknown',
            
            // Original Consumer API data for reference
            originalData: item
        }));
        
        console.log('🔄 Transformed Consumer API get_userdata response:', {
        ...
        return { 
            ...
            total_count: data.count || transformedData.length
        };
    } catch (error) {
