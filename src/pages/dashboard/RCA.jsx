...
export function RCA() {
  ...
  const generateAIReport = async () => {
    ...
    try {
      ...
      const newReport = {
        ...
        type: 'ai-categories',
        title: `AI Categories Analysis - ${ministry}`,
        ministry: ministry,
        dateRange: '2016-08-01 to 2016-08-31',
        timestamp: Date.now(),
        data: {
          hierarchicalCategories: [{
            count: aiTreeData.count,
            children: aiTreeData.children
          }]
        }
      }
      
      const updatedHistory = [newReport, ...aiReportHistory]
      setAiReportHistory(updatedHistory)
      localStorage.setItem('aiReportHistory', JSON.stringify(updatedHistory))
  ...
  // Handle history item click - apply filteration for AI categories
  const handleHistoryClick = (report) => {
    if (report.type === 'ai-categories') {
      // Apply the same filteration as Generate AI Report
      console.log('🔄 Applying AI Categories from history:', report)
      
      // Set the ministry and financial term from the report
      if (report.ministry) {
        setMinistry(report.ministry)
      }
      
      // Convert AI categories to tree structure for main visualization
      const aiTreeData = {
        count: report.data?.hierarchicalCategories?.[0]?.count || 9494,
        topicname: 'AI Categories Root',
        children: report.data?.hierarchicalCategories?.[0]?.children?.map((category, index) => ({
      ...
      toast.success('AI Categories loaded from history!')
      return
    }
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {aiReportHistory.length > 0 ? (
                    ...
                    // Demo samples when no history exists
                    [
                      {
                        id: 'demo-1',
                        type: 'ai-categories',
                        title: 'Consumer Services Analysis',
                        ministry: 'DOCAF',
                        dateRange: '2016-08-01 to 2016-08-31',
                        count: 2938,
                      ...
                      },
                      {
                        id: 'demo-2', 
                        type: 'ai-categories',
                        title: 'Telecom Complaints Pattern',
                        ministry: 'DOCAF',
                        dateRange: '2016-08-01 to 2016-08-31',
                        count: 1547,
                      ...
                      <Card
                        key={demoReport.id}
                        onClick={() => {
                          toast.info('This is a demo sample. Generate AI Report to create real analysis.');
                        }}
                        className={`cursor-pointer transition-all duration-200 hover:shadow-md transform hover:scale-[1.02] ${
                          isDark 
                            ? 'bg-gradient-to-br from-gray-800/50 to-gray-700/50 border-gray-600 hover:from-gray-700/60 hover:to-gray-600/60' 
                            : 'bg-gradient-to-br from-gray-100 to-gray-50 border-gray-300 hover:from-gray-200 hover:to-gray-100'
                        } border opacity-75`}
                      >
                        <CardBody className="p-4">
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              />
                              <Typography variant="small" color="gray" className="text-xs">
                                {new Date(demoReport.timestamp).toLocaleDateString()}
                              </Typography>
                            </div>
                            
                            <Typography variant="small" className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                              {demoReport.title}
                            </Typography>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <Typography variant="small" color="gray" className="text-xs">
                                {demoReport.count.toLocaleString()} cases
                              </Typography>
                            </div>
                            
                            <Typography variant="small" color="gray" className="text-xs">
                              {demoReport.dateRange}
                            </Typography>
                          </div>
                        </CardBody>
                      </Card>
